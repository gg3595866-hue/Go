  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 sm:gap-4">
        {/* Time/Status Column */}
        <div className="flex flex-col items-center justify-center w-10 sm:w-14 flex-shrink-0">
          {isFinished && (
          ...
          {isScheduled && (
            <span className="text-xs sm:text-sm font-medium tabular-nums" data-testid="text-match-time">
              {match.time}
            </span>
          )}
        </div>
        ...
        {/* Teams Section */}
        <div className="flex-1 flex items-center justify-between gap-1.5 sm:gap-4 min-w-0">
          {/* Home Team */}
          <div className="flex items-center gap-1.5 sm:gap-2 flex-1 min-w-0 max-w-[35%] sm:max-w-none">
            {match.homeTeamLogo ? (
              <img
            ...
            <span className="text-xs sm:text-sm font-medium truncate" data-testid={`text-team-${match.homeTeam}`}>
              {match.homeTeam}
            </span>
          </div>

          {/* Score */}
          <div className="flex flex-col items-center gap-0.5 min-w-[40px] sm:min-w-[60px] flex-shrink-0">
            {match.homeScore !== null && match.awayScore !== null ? (
              <>
                <div className="flex items-center gap-1 sm:gap-2 text-base sm:text-xl font-bold tabular-nums" data-testid="text-score">
                  <span>{match.homeScore}</span>
                  <span className="text-muted-foreground">-</span>
                  <span>{match.awayScore}</span>
                </div>
                {match.homeHalfScore !== null && match.awayHalfScore !== null && (
                  <div className="text-[10px] sm:text-xs text-muted-foreground tabular-nums" data-testid="text-halftime-score">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 flex-1 min-w-0 max-w-[35%] sm:max-w-none">
            ...
            <span className="text-xs sm:text-sm font-medium truncate" data-testid={`text-team-${match.awayTeam}`}>
              {match.awayTeam}
            </span>
          </div>
        </div>
