...
import { Brain, CheckCircle2, TrendingUp, Trash2 } from "lucide-react";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
import type { BasketballModelMetadata } from "@shared/schema";
...
export default function BasketballTrainingPage() {
  ...
  const { data: models = [], isLoading: modelsLoading } = useQuery<BasketballModelMetadata[]>({
    queryKey: ['/api/basketball/ml/models'],
  });

  const { data: activeModel } = useQuery<BasketballModelMetadata>({
    queryKey: ['/api/basketball/ml/models/active'],
  });

  const trainMutation = useMutation({
    mutationFn: async () => {
      return await apiRequest('/api/basketball/ml/train', {
        method: 'POST',
  ...
  const activateMutation = useMutation({
    mutationFn: async (modelId: number) => {
      return await apiRequest(`/api/basketball/ml/models/${modelId}/activate`, {
        method: 'POST',
      });
    },
    onSuccess: () => {
      toast({
        ...
        description: "The selected basketball model is now active for predictions",
      });
      queryClient.invalidateQueries({ queryKey: ['/api/basketball/ml/models'] });
      queryClient.invalidateQueries({ queryKey: ['/api/basketball/ml/models/active'] });
    },
  ...
  const deleteAllMutation = useMutation({
    mutationFn: async () => {
      return await apiRequest('/api/basketball/ml/models', {
        method: 'DELETE',
      });
    },
    onSuccess: () => {
      toast({
        ...
        description: "All saved basketball models have been deleted successfully",
      });
      queryClient.invalidateQueries({ queryKey: ['/api/basketball/ml/models'] });
      queryClient.invalidateQueries({ queryKey: ['/api/basketball/ml/models/active'] });
    },
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-3xl font-bold" data-testid="text-page-title">Basketball Model Training</h1>
          <p className="text-muted-foreground">Train neural network models for basketball match prediction</p>
        </div>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Configure basketball model training parameters</CardDescription>
          </CardHeader>
          ...
          <CardHeader>
            <div className="flex justify-between items-start">
              <div>
                ...
                <CardDescription>Trained basketball models and their performance</CardDescription>
              </div>
              {models.length > 0 && (
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      <AlertDialogTitle>Delete All Basketball Models?</AlertDialogTitle>
                      <AlertDialogDescription>
                        This will permanently delete all {models.length} saved basketball model(s). This action cannot be undone.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
          ...
          <CardContent>
            {modelsLoading ? (
              ...
              <div className="text-center py-8">
                ...
                <p className="text-muted-foreground">No basketball models trained yet</p>
                <p className="text-sm text-muted-foreground">Start training to create your first basketball model</p>
              </div>
            ) : (
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              Active Basketball Model Details
            </CardTitle>
          </CardHeader>
          <CardContent>
