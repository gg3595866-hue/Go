(function() {
    ...
    switch (msg.type) {
      ...
      case 'connect':
        if (msg.data && msg.data.url) {
          ...
          if (msg.data.token) authToken = msg.data.token;
          chrome.storage.local.set({ serverUrl: serverUrl, authToken: authToken });
          connectToServer();
          setTimeout(function() {
      ...
      case 'set_token':
        authToken = msg.data && msg.data.token ? msg.data.token : null;
  ...
  function connectToServer() {
    ...
    if (!serverUrl) return;
    if (!authToken) {
      console.log('[Witch BG] No auth token — cannot connect. Login first.');
      broadcastToPopup({ type: 'auth_required' });
      return;
    }

    var wsUrl = serverUrl
      .replace(/^https?:\/\//, function(m) { return m === 'https://' ? 'wss://' : 'ws://'; })
      .replace(/\/$/, '') + '/ws/witch?source=extension&token=' + encodeURIComponent(authToken);
  ...
  // ============================================================
  chrome.storage.local.get(['serverUrl', 'isConnected', 'lastState', 'authToken'], function(result) {
    if (result.lastState) {
